...
fun! s:PathHash(val)
    return substitute(system("shasum", a:val), " .*", "", "")
endfun
...
"        let g:autotags_ctags_global_include = "/Applications/Xcode.app/Contents/Developer/Platforms/MacOSX.platform/Developer/SDKs/MacOSX10.11.sdk/usr/include/* "
    endif
    ...
    let s:cscope_file_pattern = '.*\.(cpp|cc|cxx|m|hpp|hh|h|hxx|c|idl)$'
...
fun! s:AutotagsGenerate(sourcedir, tagsdir)
    ...
    echomsg system("cd '" . l:cscopedir . "' && nice -15 find -E '" . a:sourcedir . "' " .
        \ "-not -regex '.*\\.git.*' -regex '" . s:cscope_file_pattern . "' -print > cscope.files")
